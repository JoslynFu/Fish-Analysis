...
We include only the data before 2008 because the later record of catches is incomplete.
```{r}
collapse <- fish %>%
...
collapse %>%
  ...
  pivot_longer(-tsyear) %>%
  ggplot(aes(x = tsyear, y = value, color = name)) + geom_point() + ggtitle("Percent Species Collapsed") + xlab("Year") + ylab("Species Collapsed")
